...
export async function scanUpcomingLineups({minutes=180,limit=12}={}){
  ...
  const rows=await mapLimit(events,2,async e=>{
    ...
    const ctx=await buildMatchContext(String(e.id),e.homeTeam.id,e.awayTeam.id,e.startTimestamp,hh.events,aa.events,e.dataSource||"auto"),raw=analyze(e.homeTeam.id,e.awayTeam.id,hh.events,aa.events,e.startTimestamp,ctx),analysis=applyCalibration(raw,cal.rows),withImpact=await attachLineupImpact(String(e.id),e.startTimestamp,analysis);
