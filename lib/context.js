...
async function opponentScheduleStrength(events,teamId,depth=3){
  ...
  const rows=await Promise.all(chosen.map(async o=>{
    try{const h=await history(o.id,o.ts,6);return {...o,...recentPower(h.events,o.id)}}catch{return {...o,rating:50,n:0}}
  }));
...
export async function buildMatchContext(eventId,homeId,awayId,targetTs,homeEvents=[],awayEvents=[]){
  let s=null,league=null;
  try{s=eventId?await summary(eventId):null;league=s?.header?.league||null}catch{}
  ...
  const [env,elo,global,hs,as,hx,ax]=await Promise.all([
    ...
    opponentScheduleStrength(homeEvents,homeId,3),
    opponentScheduleStrength(awayEvents,awayId,3),
    teamAdvancedContext(homeEvents,homeId,s,5),
    teamAdvancedContext(awayEvents,awayId,s,5)
  ]);
