...
const cache=new Map(),summaryPending=new Map(),clamp=(v,a,b)=>Math.max(a,Math.min(b,v));
...
async function json(url,timeout=12000){
  ...
  try{const r=await fetch(url,{signal:c.signal,headers:{accept:"application/json,text/plain,*/*","user-agent":"Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 Version/18.0 Mobile/15E148 Safari/604.1",referer:"https://www.espn.com/","accept-language":"tr-TR,tr;q=0.9,en;q=0.7"}});if(!r.ok)throw new Error("HTTP "+r.status);return await r.json()}
...
async function summary(eventId){
  ...
  if(summaryPending.has(k))return summaryPending.get(k);
  const task=(async()=>{const raw=await json("https://site.api.espn.com/apis/site/v2/sports/soccer/all/summary?event="+eventId,18000),d=compactSummary(raw),done=!!d?.header?.event?.completed,starterCount=(d?.rosters||[]).reduce((n,r)=>n+(r.roster?.length||0),0),ttl=done?21600000:starterCount>=18?120000:180000;cs(k,d,ttl);return d})().finally(()=>summaryPending.delete(k));
  summaryPending.set(k,task);return task
}
...
export async function buildMatchContext(eventId,homeId,awayId,targetTs,homeEvents=[],awayEvents=[],source="auto",identity={}){
  let s=null,league=null,summaryError=null;
  try{s=eventId&&source!=="sofascore"?await summary(eventId):null;league=s?.header?.league||null}catch(error){summaryError=String(error?.message||error)}
  ...
  return {
    teams:{home:hb?.team?.displayName||hc?.name||String(identity?.homeName||""),away:ab?.team?.displayName||ac?.name||String(identity?.awayName||"")},
    diagnostics:{currentSummaryAvailable:!!s,marketAvailable:!!extractMarket(s),identityFallback:!s||(!hc?.name&&!hb?.team?.displayName),summaryError},
