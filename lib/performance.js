...
export async function backfillPerformanceDate(date,{limit=8}={}){
  ...
  const rows=await mapLimit(matches,2,async e=>{
    const [hh,aa]=await Promise.all([history(e.homeTeam.id,e.startTimestamp,10),history(e.awayTeam.id,e.startTimestamp,10)]);if((hh.events?.length||0)<5||(aa.events?.length||0)<5)return {skip:true};
    const ctx=await buildMatchContext(String(e.id),e.homeTeam.id,e.awayTeam.id,e.startTimestamp,hh.events,aa.events),a=analyze(e.homeTeam.id,e.awayTeam.id,hh.events,aa.events,e.startTimestamp,ctx);
