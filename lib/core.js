...
function espnEvent(e){
  ...
  const completed=!!c?.status?.type?.completed,country=c?.venue?.address?.country||"Dünya",leagueName=e?.league?.name||e?.league?.shortName||prettySlug(e?.season?.slug)||"Futbol",leagueId=String(e?.uid||"").match(/(?:^|~)l:([^~]+)/)?.[1]||null;
  ...
  return {
    ...
    homeScore:{current:hs,normaltime:hs,display:String(hs)},awayScore:{current:as,normaltime:as,display:String(as)},
    tournament:{name:leagueName,category:{name:country},uniqueTournament:{name:leagueName,id:leagueId}}
  }
}
...
const ESPN_HEADERS={"user-agent":"Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 Version/18.0 Mobile/15E148 Safari/604.1","referer":"https://www.espn.com/","accept-language":"tr-TR,tr;q=0.9,en;q=0.7"};
...
function countMarkets(context,lh,la){
  ...
  const blend=(own,opp,trend,fallback)=>{
    ...
    const base=(Number.isFinite(a)?0.48*a:0)+(Number.isFinite(b)?0.37*b:0)+(Number.isFinite(t)?0.15*t:0),w=(Number.isFinite(a)?0.48:0)+(Number.isFinite(b)?0.37:0)+(Number.isFinite(t)?0.15:0);
...
export function analyze(home,away,he,ae,targetTs,context={}){
  ...
  const shrink=q>=8?0.82:q>=5?0.70:0.56;let lh=baseH*(1-shrink)+rawH*shrink,la=baseA*(1-shrink)+rawA*shrink;
  const longEdge=cl((ha.ppg-aa.ppg)/3,-1,1),recentEdge=cl((ha.momentum.ppg-aa.momentum.ppg)/3,-1,1),ratingEdge=cl((hp-ap)/30,-1,1),eloEdge=cl((Number(elo.home||1500)-Number(elo.away||1500))/220,-1,1),eloW=eloCoverage>=6?0.20:eloCoverage>=3?0.11:0,globalEdge=cl(Number(global.edge||0),-1,1),globalW=globalCoverage>=10?0.14:globalCoverage>=6?0.08:0,baseEdge=.43*longEdge+.21*recentEdge+.17*ratingEdge,edge=cl((baseEdge+eloW*eloEdge+globalW*globalEdge)/(.81+eloW+globalW),-1,1);
  ...
  const trendO=(ha.over25+aa.over25+hh.over25+av.over25)/4,trendB=(ha.btts+aa.btts+hh.btts+av.btts)/4,leagueO=Number(env?.over25||0)/100,leagueB=Number(env?.btts||0)/100,trendOW=envN>=12?0.22:0,trendBW=envN>=12?0.18:0,formPush=cl(edge*.05,-.05,.05);
