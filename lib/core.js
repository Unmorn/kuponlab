...
async function getJson(url,timeout=12000){const c=new AbortController(),t=setTimeout(()=>c.abort(),timeout);try{const r=await fetch(url,{signal:c.signal,headers:{"accept":"application/json,text/plain,*/*","accept-language":"tr-TR,tr;q=0.9,en;q=0.8"}});if(!r.ok)throw new Error("HTTP "+r.status);return await r.json()}finally{clearTimeout(t)}}
...
function espnEvent(e){
  ...
  const h=arr.find(x=>x.homeAway==="home")||arr[0];
  const a=arr.find(x=>x.homeAway==="away")||arr[1];
  if(!h?.team?.id||!a?.team?.id)return null;
  const completed=!!c?.status?.type?.completed;
  const country=c?.venue?.address?.country||"Dünya";
  const leagueName=e?.league?.name||e?.league?.shortName||prettySlug(e?.season?.slug)||"Futbol";
  const hs=n(h?.score?.value??h?.score,0),as=n(a?.score?.value??a?.score,0);
  return {
    id:Number(e.id)||e.id,
    startTimestamp:Math.floor(new Date(e.date).getTime()/1000),
    status:{type:completed?"finished":"scheduled",code:completed?100:0},
    homeTeam:{id:Number(h.team.id)||h.team.id,name:h.team.displayName||h.team.name||h.team.location||"Ev"},
    awayTeam:{id:Number(a.team.id)||a.team.id,name:a.team.displayName||a.team.name||a.team.location||"Deplasman"},
    homeScore:{current:hs,normaltime:hs,display:String(hs)},
    awayScore:{current:as,normaltime:as,display:String(as)},
...
async function espnFixtures(date){
  ...
  const d=await getJson("https://site.api.espn.com/apis/site/v2/sports/soccer/all/scoreboard?dates="+compact+"&limit=1000",15000);
  return (d.events||[]).map(espnEvent).filter(Boolean)
}
async function espnHistory(team,targetTs,limit=10){
  const cutoff=Number(targetTs)||Math.floor(Date.now()/1000),year=new Date(cutoff*1000).getUTCFullYear();
  let ev=[];
  for(const y of [year,year-1,year-2]){
    try{
      const d=await getJson("https://site.api.espn.com/apis/site/v2/sports/soccer/all/teams/"+team+"/schedule?season="+y,15000);
      ev=ev.concat((d.events||[]).map(espnEvent).filter(Boolean).filter(e=>e.status?.type==="finished"&&e.startTimestamp<cutoff));
      const uniq=[...new Map(ev.map(e=>[e.id,e])).values()];
...
export async function fixtures(date){
  const k="f:"+date,c=cg(k);if(c)return {cached:true,events:c,source:"ESPN"};
  const ev=await espnFixtures(date);
  cs(k,ev,300000);
  return {cached:false,events:ev,source:"ESPN"};
}
export async function last(team){
  const k="t:"+team,c=cg(k);if(c)return {cached:true,events:c,source:"ESPN"};
  const ev=await espnHistory(team,Math.floor(Date.now()/1000),10);
  cs(k,ev,86400000);
  return {cached:false,events:ev,source:"ESPN"};
}
export async function history(team,targetTs,limit=10){
  const day=Math.floor((Number(targetTs)||0)/86400),k="h:"+team+":"+day+":"+limit,c=cg(k);if(c)return {cached:true,events:c,source:"ESPN"};
  const ev=await espnHistory(team,targetTs,limit);
  cs(k,ev,21600000);
  return {cached:false,events:ev,source:"ESPN"};
}
