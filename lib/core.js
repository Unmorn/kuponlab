...
async function espnFixtures(date){
  ...
  let lastErr=null;
  for(const url of urls){try{const d=await getJson(url,9000,ESPN_HEADERS);if(Array.isArray(d?.events))return d.events.map(espnEvent).filter(Boolean)}catch(e){lastErr=e}}
...
async function espnHistory(team,targetTs,limit=10){
  ...
  for(const y of [year,year-1,year-2]){
    try{
      const d=await getJson("https://site.api.espn.com/apis/site/v2/sports/soccer/all/teams/"+team+"/schedule?season="+y,9000,ESPN_HEADERS);worked=true;
