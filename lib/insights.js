...
export async function dailyInsights(date,{extended=false}={}){
  ...
  const analyses=(await mapLimit(analysisPool,3,async x=>{
    ...
    if(!(h.events||[]).length||!(a.events||[]).length)return null;
    let context={};
    try{context=await buildMatchContext(String(e.id),e.homeTeam.id,e.awayTeam.id,e.startTimestamp,h.events,a.events,source)}catch{}
    const raw=build(e.homeTeam.id,e.awayTeam.id,h.events,a.events,e.startTimestamp,context||{}),base={...applyCalibration(raw,cal.rows||[]),backtestMatches:Number(cal.total||0)};
    let enriched=base;
    try{enriched=await enrichWithIddaa(base,e.homeTeam.name,e.awayTeam.name,e.startTimestamp,{trackOdds:false})}catch{enriched={...base,iddaa:{matched:false,reason:"unavailable"}}}
    let analysis=enriched;
    try{analysis=applyPerformanceToAnalysis(enriched,performance)}catch{}
