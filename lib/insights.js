...
export async function dailyInsights(date,{extended=false,baseUrl=""}={}){
  ...
  const analyses=(await mapLimit(analysisPool,2,async x=>{
    ...
    if(baseUrl){
      const qs=new URLSearchParams({home:String(e.homeTeam.id),away:String(e.awayTeam.id),ts:String(e.startTimestamp),event:String(e.id),source});
      ...
      const analysis=d.analysis,marketComparison=analysis.marketComparison||{available:false},product=analysis.product||decorateAnalysis(analysis,marketComparison);
