...
export async function dailyInsights(date,{extended=false}={}){
  ...
  const pool=scheduled.slice(0,extended?60:28),pre=await mapLimit(pool,extended?6:5,async e=>{
    const [espn,iddaa]=await Promise.all([marketSnapshot(e.id),iddaaMatchSnapshot(e.homeTeam.name,e.awayTeam.name,e.startTimestamp)]);
    return {event:e,...espn,iddaa}
  });
  const withIddaa=pre.filter(x=>x?.iddaa?.matched),withFallback=pre.filter(x=>!x?.iddaa?.matched&&x?.market),withoutMarket=pre.filter(x=>!x?.iddaa?.matched&&!x?.market),analysisPool=[...withIddaa,...withFallback,...withoutMarket].slice(0,extended?36:14);
  ...
  const analyses=(await mapLimit(analysisPool,2,async x=>{
    const e=x.event,[h,a]=await Promise.all([history(e.homeTeam.id,e.startTimestamp,10),history(e.awayTeam.id,e.startTimestamp,10)]);
    const context=await buildMatchContext(String(e.id),e.homeTeam.id,e.awayTeam.id,e.startTimestamp,h.events,a.events);
    const raw=build(e.homeTeam.id,e.awayTeam.id,h.events,a.events,e.startTimestamp,context),base={...applyCalibration(raw,cal.rows),backtestMatches:cal.total};
    const enriched=await enrichWithIddaa(base,e.homeTeam.name,e.awayTeam.name,e.startTimestamp,{trackOdds:false}),analysis=applyPerformanceToAnalysis(enriched,performance);
    const marketComparison=compareMarket(analysis,context.market),product=decorateAnalysis(analysis,marketComparison);
  ...
  const v={date,extended,summary:{totalFixtures:events.length,scheduled:scheduled.length,iddaaMatches:pre.filter(x=>x?.iddaa?.matched).length,pricedMatches:pre.filter(x=>x?.iddaa?.matched||x?.market).length,analyzed:analyses.length,strongSignals:strong,suitable,valueOpportunities:opportunities.length},opportunities:opportunities.slice(0,5),marketPerformance:performance.groups,coupons,couponSets,source:"ESPN + İddaa public sportsbook + KuponLab v5.2 Ensemble",cached:false};
