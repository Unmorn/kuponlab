...
export async function dailyInsights(date,{extended=false}={}){
  ...
  const analyses=(await mapLimit(analysisPool,2,async x=>{
    const e=x.event,[h,a]=await Promise.all([history(e.homeTeam.id,e.startTimestamp,10,e.dataSource||"auto"),history(e.awayTeam.id,e.startTimestamp,10,e.dataSource||"auto")]);
    const context=await buildMatchContext(String(e.id),e.homeTeam.id,e.awayTeam.id,e.startTimestamp,h.events,a.events,e.dataSource||"auto");
