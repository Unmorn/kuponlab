...
const getCache=(k,allowExpired=false)=>{const x=cache.get(k);if(!x)return null;if(x.e>Date.now()||allowExpired)return x.v;return null};
const setCache=(k,v,ms)=>{cache.set(k,{v,e:Date.now()+ms});return v};
async function singleFlight(key,fn){
  if(pending.has(key))return pending.get(key);
  const task=Promise.resolve().then(fn).finally(()=>pending.delete(key));pending.set(key,task);return task
}
function sqlValue(v){
...
async function getJson(url,ms=20000,cacheTtl=0){
  const c=new AbortController(),t=setTimeout(()=>c.abort(),ms);
  const init={headers,signal:c.signal};if(cacheTtl>0)init.cf={cacheEverything:true,cacheTtl};
  try{const r=await fetch(url,init);if(!r.ok)throw new Error("HTTP "+r.status);return await r.json()}finally{clearTimeout(t)}
}

export async function iddaaEvents(){
  const k="events",c=getCache(k);if(c)return c;
  const stale=getCache(k,true);
  return singleFlight(k,async()=>{
    try{
      const d=await getJson(BASE+"/sportsbook/events?st=1&type=0&version=0",28000,75);
      if(!d?.isSuccess||!Array.isArray(d?.data?.events))throw new Error(d?.message||"İddaa program yanıtı geçersiz");
      return setCache(k,d.data.events,90000)
    }catch(error){if(Array.isArray(stale)&&stale.length)return stale;throw error}
  })
}

export async function iddaaProgram(date=null){
  const validDate=/^\d{4}-\d{2}-\d{2}$/.test(String(date||"")),dayStart=validDate?Math.floor(Date.parse(String(date)+"T00:00:00+03:00")/1000):null,dayEnd=dayStart==null?null:dayStart+86400;
  const rows=(await iddaaEvents()).filter(x=>Number(x?.sid||1)===1&&x?.i&&x?.hn&&x?.an&&Number(x?.d)>0&&(dayStart==null||(Number(x.d)>=dayStart&&Number(x.d)<dayEnd)));
  return {
    date:validDate?String(date):null,
    generatedAt:Date.now(),
    eventCount:rows.length,
    events:rows.map(x=>({eventId:Number(x.i),home:String(x.hn),away:String(x.an),startTimestamp:Number(x.d),mbs:Number(x.mbc||0)||null,marketCount:Array.isArray(x.m)?x.m.length:0}))
  }
}
...
export async function marketConfig(){
  ...
  return singleFlight(k,async()=>{
    try{
      const d=await getJson(BASE+"/sportsbook/get_market_config",28000,6*60*60);
      if(!d?.isSuccess||!d?.data?.m)throw new Error(d?.message||"İddaa market ayarı yanıtı geçersiz");
      return setCache(k,d.data.m,6*60*60*1000)
    }catch(error){if(stale&&Object.keys(stale).length)return stale;throw error}
  })
}
...
function sim(a,b){
  ...
  const jac=inter/Math.max(1,new Set([...A,...B]).size),inc=a.includes(b)||b.includes(a)?0.30:0;
  return Math.min(1,jac+inc)
}

export function matchIddaaRows(event,rows=[]){
...
export async function matchIddaaEvent(event){return matchIddaaRows(event,await iddaaEvents())}
...
export async function iddaaEventDetail(id){
  ...
  return singleFlight(k,async()=>{
    try{
      const d=await getJson(BASE+"/sportsbook/event/"+encodeURIComponent(id),24000,30);
      if(!d?.isSuccess||!d?.data)throw new Error(d?.message||"İddaa etkinlik yanıtı geçersiz");
      return setCache(k,d.data,45000)
    }catch(error){if(stale)return stale;throw error}
  })
}
...
export async function officialSnapshot(event,{eventId=null}={}){
  try{
    let match=null,detail=null;
    if(eventId){
      detail=await iddaaEventDetail(eventId);
      match=matchIddaaRows(event,detail?[detail]:[]);
      if(!match)return {available:false,matched:false,reason:"iddaa_hint_mismatch"}
    }else match=await matchIddaaEvent(event);
    if(!match)return {available:false,matched:false,reason:"not_in_iddaa_program"};
    detail=detail||await iddaaEventDetail(match.eventId);const markets=await enrichMarkets(detail);
...
function officialOddMeta(rows,mapped,defaultMbs=null){
  ...
  const row=rows.find(x=>String(x.marketId)===String(mapped.marketId)&&String(x.outcomeName)===String(mapped.outcomeName));
  if(!row||Number(row.status)!==1||!(Number(row.odd)>1))return null;
  const code=row.marketType+"_"+row.subType;
...
export async function enrichWithIddaa(analysis,home,away,targetTs,{trackOdds=true,eventId=null}={}){
  const event={homeTeam:{name:home},awayTeam:{name:away},startTimestamp:Number(targetTs)};
  const snap=await officialSnapshot(event,{eventId});
