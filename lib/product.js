...
export function customCoupon(items,count=8,target=10,variant=0){
  count=Math.round(clamp(Number(count)||8,2,20));target=clamp(Number(target)||10,1.5,5000);variant=Math.max(0,Math.round(Number(variant)||0));
  const avgLeg=Math.pow(target,1/count),prepared=[];
  for(const it of items){
    const opts=customCandidatePool(it).filter(x=>x.oddsType!=="iddaa"||!Number(x.mbs)||Number(x.mbs)<=count).sort((a,b)=>customQuality(b,avgLeg,variant)-customQuality(a,avgLeg,variant)).slice(0,10);
    if(opts.length)prepared.push({event:it.event,opts})
  }
  const realGroups=prepared.filter(g=>g.opts.some(x=>x.oddsType==="iddaa")),iddaaOnly=realGroups.length>=count,groups=(iddaaOnly?realGroups:prepared).map(g=>({...g,opts:iddaaOnly?g.opts.filter(x=>x.oddsType==="iddaa"):g.opts}));
  count=Math.min(count,groups.length);
  const beam=180,targetLog=Math.log(target),states=Array.from({length:count+1},()=>[]);
  states[0]=[{picks:[],logOdds:0,quality:0,families:{},leagues:{},fair:0,real:0}];
  const rankState=(s,k)=>{
    const progress=k/Math.max(1,count),expected=targetLog*progress,dist=Math.abs(s.logOdds-expected),familyPenalty=Object.values(s.families).reduce((z,n)=>z+Math.max(0,n-Math.ceil(count/3))*5,0),leaguePenalty=Object.values(s.leagues).reduce((z,n)=>z+Math.max(0,n-Math.ceil(count/3))*4,0);
    return s.quality*.16-dist*170-familyPenalty-leaguePenalty
  };
  for(const g of groups){
    const next=states.map(a=>a.slice());
    for(let k=Math.min(count-1,groups.length);k>=0;k--){
    ...
    for(let k=0;k<=count;k++)next[k]=next[k].sort((a,b)=>rankState(b,k)-rankState(a,k)).slice(0,beam);
    for(let k=0;k<=count;k++)states[k]=next[k]
  }
  const finals=(states[count]||[]).sort((a,b)=>{
  ...
  const notes=[];if(picks.length<count)notes.push("İstenen "+count+" seçim için yeterli kaliteli maç bulunamadı.");if(Math.abs(delta)>12)notes.push("Hedef orana tam yaklaşacak kaliteli kombinasyon bulunamadı; en yakın seçenek gösteriliyor.");if(iddaaOnly)notes.push("Bu kupon yalnızca şu an İddaa'da açık gerçek oranlı marketlerden oluşturuldu.");else if(health.fairCount)notes.push("Yeterli gerçek İddaa marketi olmadığı için bazı seçimlerde model-adil fallback kullanıldı; toplam yaklaşık olabilir.");
  return {profile:"custom",variant,count,target,avgLeg:+avgLeg.toFixed(2),achievedOdds:+achieved.toFixed(2),targetDeltaPct:+delta.toFixed(1),iddaaOnly,recipe:count+" maç · "+target+"x hedef",picks,mix,notes,...health,complete:picks.length===count}
}
export function targetCoupon(items,target=5,variant=0){
  const all=[];
  for(const it of items){
    const candidates=pricedCandidates(it.analysis,it.marketComparison).filter(x=>x.decimal>=1.12&&x.decimal<=2.15&&x.risk<=56&&x.probability>=50&&it.analysis.dataQuality>=52&&it.analysis.confidence>=43&&Number(it.analysis.stability||70)>=58);
    candidates.sort((a,b)=>((b.probability*1.2-b.risk*.75+b.edge*35)+(hash01(String(it.event.id)+"|"+b.key+"|T|"+variant)-.5)*7)-((a.probability*1.2-a.risk*.75+a.edge*35)+(hash01(String(it.event.id)+"|"+a.key+"|T|"+variant)-.5)*7));
  ...
  const chosen=[];let running=1;
  for(const x of all){if(chosen.length>=13)break;chosen.push(x);running*=x.decimal;if(running>=target*.92&&chosen.length>=2)break}
  const picks=chosen.map(x=>({matchId:x.event.id,match:x.event.homeTeam.name+" - "+x.event.awayTeam.name,league:x.event.tournament?.uniqueTournament?.name||x.event.tournament?.name||"Futbol",startTimestamp:x.event.startTimestamp,marketKey:x.key,pick:x.label,probability:x.probability,decimal:x.decimal,openDecimal:x.openDecimal,oddsType:"market",oddsMovePct:x.oddsMovePct,edge:+x.edge.toFixed(4),risk:x.risk,confidence:x.confidence,dataQuality:x.dataQuality,stability:x.stability,marketFamily:x.family||null,marketGroup:x.marketGroup,why:"Hedef orana giderken düşük riskli ve gerçek oranı bulunan seçimler arasından seçildi."}));
  const health=couponHealth(picks);return {target:+target,variant:Number(variant)||0,recipe:"Hedef oran · alternatif "+((Number(variant)||0)+1),picks,...health,reached:health.totalOdds?health.totalOdds>=target*.92:false}
}
