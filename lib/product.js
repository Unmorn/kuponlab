...
export function customCoupon(items,count=8,target=10,variant=0){
  ...
  for(const it of items){
    const opts=customCandidatePool(it).filter(x=>x.oddsType!=="iddaa"||!Number(x.mbs)||Number(x.mbs)<=requestedCount).sort((a,b)=>customQuality(b,avgLeg,variant)-customQuality(a,avgLeg,variant)).slice(0,3);
  ...
  const beam=4,targetLog=Math.log(target),states=Array.from({length:effectiveCount+1},()=>[]);
